...
### Assigning intermediate value to symbol

Intermediate values in the middle of a pipeline can be assigned to symbols, which is done by the syntax using `=`. For example,

`x %>>% (~ y)` indicates that the input value `x` will be assigned to `y` as side-effect (that's why it starts with `~`).
...
`x %>>% (~ y = f(.))` indicates that the value of `f(x)` will be assigned to `y` as a side-effect, which returns the input value `x` and thus does not influence the pipeline.
...
One is allowed to use lambda expression on the right-hand side of `=`.
...
In other cases, one may need to assign to a symbol the value of an intermediate step of the pipeline. To do this, one need to remove `~` before the enclosed expression to indicate that its value should be piped.
